...
if (isset($_SESSION['user'])) {
    ...
    if ($get_session_user_query->execute()) {
        ...
        if ($get_promotion_details_query->execute()) {
            ...
            if ($get_promotion_media_query->execute() && $get_user_query->execute() && $get_promotion_product_boutique_query->execute() && $get_saved_promotion_query->execute() && $get_participant_promotion_query->execute()) {
...
<?php 
    ...
    if ($get_promotion_boutique_product_query->execute()) {
        $i = 0;
        while ($get_promotion_boutique_product_row = $get_promotion_boutique_product_query->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($get_boutique_product_query->execute()) {
...
<?php 
    ...
    if ($get_promotion_product_query->execute()) {
        while ($get_promotion_product_row = $get_promotion_product_query->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($get_fist_promotion_product_media_query->execute()) {
                ...
                if ($get_promotion_product_media_query->execute()) {
?>
...
<?php 
    ...
    if ($get_promotion_details_query->execute()) {
        ...
        $id_user_prm = $get_promotion_details_row['id_user'];
        $get_user_query = $conn->prepare("SELECT id_user,img_user,nom_user FROM utilisateurs WHERE id_user = $id_user_prm");
        if ($get_promotion_media_query->execute() && $get_user_query->execute() && $get_promotion_product_boutique_query->execute()) {
            $get_promotion_media_row = $get_promotion_media_query->fetch(PDO::FETCH_ASSOC);
?>
<div class="promotion-details-top">
    <div class="cancel-promotion-details-resp" id="cancel_promotion_details_resp">
