...
if (isset($_SESSION['user'])) {
    ...
    if ($type_filter == 'today') {
        if(!empty($debut_evn)){
            ...
            $get_evenement_query = $conn->prepare("SELECT * FROM evenements WHERE DATE(date_evn) = '$date_debut_evn' ORDER BY id_evn DESC");
        }
        else{
            echo 0;
        }
        ...
        if(!empty($debut_evn) && !empty($fin_evn)){
            ...
            $get_evenement_query = $conn->prepare("SELECT * FROM evenements WHERE (DATE(date_evn) BETWEEN '$date_debut_evn' AND '$date_fin_evn') ORDER BY id_evn DESC");
        }
        else{
            echo 0;
        }
        ...
        $get_evenement_query = $conn->prepare("SELECT * FROM evenements WHERE titre_evn LIKE '%$text%' OR type_evn LIKE '%$text%' OR description_evn LIKE '%$text%' ORDER BY id_evn DESC");
    }
    else if ($type_filter == 'filter') {
        if (!empty($type_evn) || !empty($debut_evn) || 
            !empty($fin_evn) || !empty($ville_evn) || !empty($commune_evn)) {
            $where = "WHERE ";
            if(!empty($type_evn)){
                $where .= "typr_evn = '$type_evn' AND ";
            }
            if(!empty($debut_evn)){
                $date_debut_evn = date("Y-m-d", strtotime($debut_evn));
