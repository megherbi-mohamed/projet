...
if ($publication_query->execute()) {
    $publication_row = $publication_query->fetch(PDO::FETCH_ASSOC);
    $publication_media_query = $conn->prepare("SELECT * FROM publications_media WHERE id_pub = $id_pub"); 
    if ($publication_media_query->execute()) {
        ...
        if ($publication_comment_query->execute()) {
            $num_like_pub_query = $conn->prepare("SELECT id_j,id_user FROM jaime_publication WHERE id_pub = $id_pub"); 
            if ($num_like_pub_query->execute()) {
?>
