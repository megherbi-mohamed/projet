...
if ($get_session_idSender_query->execute()) {
    ...
    $verify_cle_query = $conn->prepare("SELECT message_cle FROM messages_cles WHERE id_recever = $id_user AND id_sender = $id_btq OR id_recever = $id_btq AND id_sender = $id_user");
    if ($verify_cle_query->execute()) {
        if($verify_cle_query->rowCount() > 0){
            ...
            $message_cle_query->bindParam(':id_sender',$id_btq);
