...
$date_prm =  date("Y-m-d");
$create_promotion_query = $conn->prepare("UPDATE promotions SET date_prm = '$date_prm', etat = 0 WHERE id_prm = $id_prm AND id_user = $id_user");
$create_product_boutique_promotion_query = $conn->prepare("UPDATE produit_boutique_promotion SET etat = 0 WHERE id_prm = $id_prm");
if ($create_promotion_query->execute() && $create_product_boutique_promotion_query->execute()){
