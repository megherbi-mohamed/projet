...
$create_promotion_query = $conn->prepare("UPDATE promotions SET titre_prm = '$titre_prm', categorie_prm = '$categorie_prm', sous_categorie_prm = '$sous_categorie_prm', lieu_prm = '$lieu_prm', adresse_prm = '$adresse_prm', 
latitude_prm = $latitude_prm, longitude_prm = $longitude_prm, date_debut_prm = '$date_debut_prm', date_fin_prm = '$date_fin_prm', description_prm = '$description_prm', date_prm = '$date_prm', etat = 0 WHERE id_prm = $id_prm AND id_user = $id_user");

$get_product_promotion_query = $conn->prepare("SELECT id_prd_prm FROM produit_boutique_promotion WHERE id_prm = $id_prm");
...
if ($get_product_promotion_query->rowCount() == 0) {
    ...
    $create_promotion_product_query = $conn->prepare("UPDATE produit_promotion SET nom_prd = '$nom_prd', reference_prd = '$reference_prd', quantite_prd = '$quantite_prd',
    ...
    if ($create_promotion_query->execute() && $create_promotion_product_query->execute()) {
    ...
    $id_prd = htmlspecialchars($_POST['id_prd']);
    $delete_promotion_product_query = $conn->prepare("DELETE FROM produit_promotion WHERE id_prm = $id_prm AND id_prd = $id_prd");
    if ($create_promotion_query->execute() && $delete_promotion_product_query->execute()) {
        $update_promotion_media_query = $conn->prepare("UPDATE promotions_media SET etat = 0 WHERE id_prm = $id_prm");
        $update_promotion_product_query = $conn->prepare("UPDATE produit_boutique_promotion SET etat = 0 WHERE id_prm = $id_prm");
        if ($update_promotion_media_query->execute() && $update_promotion_product_query->execute()) {
            echo 1;
        }else{
            echo 0;
        }
