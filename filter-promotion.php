...
if ($type_filter == 'today') {
    if(!empty($debut_prm)){
        ...
        $get_promotion_query = $conn->prepare("SELECT * FROM promotions WHERE date_prm = '$date_debut_prm' ORDER BY id_prm DESC");
    }
    else{
        echo 0;
    }
    ...
    if(!empty($debut_prm) && !empty($fin_prm)){
        ...
        $get_promotion_query = $conn->prepare("SELECT * FROM promotions WHERE (date_prm BETWEEN '$date_debut_prm' AND '$date_fin_prm') ORDER BY id_prm DESC");
    }
    else{
        echo 0;
    }
...
<div class="promotion-overview">
    ...
    <div class="promotion-overview-right">
        ...
        <div class="promotion-overview-right-middle">
            ...
            <p><?php echo $get_promotion_row['ville_prm'].', '.$get_promotion_row['commune_prm'] ?></p>
        </div>
