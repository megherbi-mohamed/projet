...
if($row = mysqli_fetch_assoc($result)){
    if ($row['code'] == $code_verification) {
        ...
        $get_preuser_row = mysqli_fetch_assoc($get_preuser_result);

        $nom_user = $get_preuser_row['nom_user'];
        $type_user = $get_preuser_row['type_user'];
        $tlph_user = $get_preuser_row['tlph_user'];
        $email_user = $get_preuser_row['email_user'];
        $mtp_user = $get_preuser_row['mtp_user'];

        $set_preuser_query = "INSERT INTO utilisateurs (type_user,nom_user,email_user,tlph_user,mtp_user)
                                                VALUES ('$type_user','$nom_user','$email_user','$tlph_user','$mtp_user')";
