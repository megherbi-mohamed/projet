...
$get_abn_query = $conn->prepare("SELECT * FROM abonnes WHERE id_abn_user = $id_user ORDER BY id_a DESC");
...
$get_abn_inf_query = $conn->prepare("SELECT id_user AS id, type_user AS type_u, img_user AS img, nom_user AS nom FROM utilisateurs WHERE id_user = {$get_abn_row['id_user']}
                                    UNION SELECT id_btq AS id, type_user AS type_u,logo_btq AS img, nom_btq AS nom FROM boutiques WHERE id_btq = {$get_abn_row['id_user']}");
