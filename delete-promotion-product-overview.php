...
if ($delete_promotion_product_query->execute()) {
    $get_promotion_prd_media_query = $conn->prepare("SELECT media_url FROM prm_produits_media WHERE id_prm = $id_prm AND id_prd = $id_prd AND etat = 1");
    if($get_promotion_prd_media_query->execute()) {
        ...
        $delete_promotion_prd_media_query = $conn->prepare("DELETE FROM prm_produits_media WHERE id_prm = $id_prm AND id_prd = $id_prd AND etat = 1");
    ...
    <div class="create-publication-bottom create-promotion-product-bottom">
        <div class="create-promotion-product-bottom-container" id="create_promotion_product_bottom_container">
            ...
            <div class="product-promotion-overview" id="product_promotion_overview_<?php echo $i ?>">
                ...
                <div id="delete_promotion_product_loader_<?php echo $i ?>" class="center"></div>
            </div>
            <?php } ?>
            </div>
            <?php } ?>
